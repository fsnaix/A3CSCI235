...
vector<Point2D> DDA(Point2D p1, Point2D p2) {
    ...
    float dx = x2 - x1;
    float dy = y2 - y1;
    int steps = std::max(std::abs(dx), std::abs(dy)); // Calculate the number of steps required for the line

    float xInc = dx / steps; // Calculate the x increment
    float yInc = dy / steps; // Calculate the y increment

    float x = x1;
    float y = y1;

    // Push the starting point
    result.push_back(Point2D(std::round(x), std::round(y)));

    // Loop to generate the line points
    for (int i = 0; i < steps; ++i) {
        x += xInc;
        y += yInc;
        // Round the x and y values to ensure the points are accurately placed on the grid
        result.push_back(Point2D(std::round(x), std::round(y)));
    }
